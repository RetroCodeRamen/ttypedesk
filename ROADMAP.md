...
- [x] Out-of-process App SDK — NDJSON over stdio (`internal/surface.ExtAppSurface`), not a Unix socket: simpler than managing a socket path/lifecycle per app, and stdin/stdout are already there for free on any spawned process. Covers the same lifecycle `uiapp.App` has (init → key/mouse/resize/focus → draw → close) plus the full `Host` RPC surface — full spec: [docs/extapp.md](docs/extapp.md), reference implementation: [`cmd/extapp-hello`](cmd/extapp-hello/main.go). Launch with `extapp:/path/to/binary [args]`, same convention as `pty:`/`bridge:`. Scoped down from the original sketch: v1 requires every `screen_diff` to be a full-grid redraw (no partial-rect diffing yet). `SaveCredential`/`LoadCredential`/`PickFile`/`ClipboardGet`/`ClipboardSet` are request/response pairs correlated by a `req_id` on the envelope (added to `internal/proto.Envelope` for exactly this); `PlayAudio` is fire-and-forget (`play_audio`/`audio_chunk`/`stop_audio`) with the actual blocking-as-backpressure consumption kept off the shared render loop — see `ExtAppSurface.readLoop`'s audio fast path. Out-of-process apps now have full parity with in-process `uiapp.Host`.
- [x] **Comprehensive Host / App API** — `uiapp.Host` gained `PlayAudio` (`internal/audio`, wraps `oto/v3`, fixed 48kHz/stereo output — no per-call resampling), `uiapp.NewMediaClock()` (play/pause/position, no Host needed — pure local timing), `PickFile` (`apps/filepicker`, a small modal browser, not a second Files app), `SaveCredential`/`LoadCredential` (`internal/credstore`, one file per key under `~/.config/ttypedesk/credentials/`, 0600), and `ClipboardGet`/`ClipboardSet` (`internal/clip` — the same clipboard the desktop's own Ctrl+Shift+C/paste keybindings use, so an app and the user share writes). "Background workers" needs no dedicated API — apps just spawn goroutines directly, same as any other Go code; prerequisite for media & chat apps below is now in place
