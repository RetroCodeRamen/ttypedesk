...
- `FrameAudio` — opt-in (Settings → Audio streaming): raw PCM captured on the host via `internal/audiocap` (a `parec` subprocess against the default sink's monitor), forwarded as chunks and played client-side via `internal/audio.Play`. Muxed onto the same connection as `FrameDiff`/`FrameJSON` through a mutex-guarded writer (`internal/attach`'s `frameWriter`), so UI and sound share one socket. Design details: [audio-stream.md](audio-stream.md).
